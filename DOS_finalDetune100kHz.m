...
%     [f1a,test] = fit(log(X_sub_prime),Y_sub_prime,'gauss1');
    ...
    energy_width_detune(k,1) = kB*T(k,1)*log((2*nu_0{1,1}(k,1))/freq_width_detune(k,1));
    energy_width_detune(k,2) = kB*T(k,1)*log(2*nu_0{1,1}(k,1)/freq_width_detune(k,2));
...
for ii = 1
    ...
    x = xlabel('N_{t} (\times 10^{19} cm^{-3}eV^{-1})');
    set(x,'FontName','Calibri');
    set(gca,'FontSize',46);
%     set(gca,'xscale','log');
    xlim([0 10]);
...
for int_idx = 1:length(ew_q)
  ...
  old_agg_detune(1,int_idx) = mean(abs(old_spline_detune(12:15,int_idx))); 
  old_agg_detune(2,int_idx) = std(abs(old_spline_detune(12:15,int_idx)))/length(12:15);

  new_agg_detune(1,int_idx) = mean(abs(new_spline_detune(12:15,int_idx))); 
  new_agg_detune(2,int_idx) = std(abs(new_spline_detune(12:15,int_idx)))/length(12:15);
...
%     
    set(gcf, 'color','white', 'Position',[1 -80 1600 900], 'PaperPosition', [.25 .25 10 8], 'inverthardcopy','off')
    frameName = [strcat('DOS_aggregate_old_1kHz',num2str(Va(1,ii)*1000),'mV_detune')];
    print(gcf, '-dpng', strcat(figuresdir4,'\',frameName),'-r0');
    savefig(gcf, fullfile(figuresdir4,frameName),'compact');
...
%     set(gca,'ytick',[1e17  1e18]);
    ...
    set(gcf, 'color','white', 'Position',[1 -80 1600 900], 'PaperPosition', [.25 .25 10 8], 'inverthardcopy','off')
    frameName = [strcat('DOS_aggregate_new_1kHz',num2str(Va(1,ii)*1000),'mV_detune')];
    print(gcf, '-dpng', strcat(figuresdir4,'\',frameName),'-r0');
    savefig(gcf, fullfile(figuresdir4,frameName),'compact');
...
ew_align = cell(1,length(12:15));
dos_align = cell(1,length(12:15));
...
for k = 12:15
    ...
    vbi_tot(2,k) = std(vbiEst{1,k})/length(1+offset_ref(1,1):offset_ref(1,2));
    
end

 disp([mean(vbi_tot(1,12:15)) std(vbi_tot(2,12:15))/length(12:15) ])
