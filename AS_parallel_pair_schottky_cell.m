...
figuresdir1 = 'C:\Users\Eric\Desktop\Admittance\ZnO-PbS\light_soak_study\IR_soak\sanity_checks\EDT_schottky_junction\try2\analysis\TAS';
...
for ii = 1:v_max; %index across V_{bias}
    ...
    hold on;
    for k = 10:t_max; %plot the remaining temperature curves and edit the plot accordingly
...
%     ylim([0 .01]);    
    hold off;
%     set(gcf, 'color','white', 'Position',[1 -80 1600 900], 'PaperPosition', [.25 .25 10 8], 'inverthardcopy','off')
%     frameName = [strcat('G_ref_short_scale_',num2str(B.(BNames{ii,1})),'mV')];
%     print(gcf, '-dpng', strcat(figuresdir1,'\',frameName),'-r0');
%     savefig(gcf, fullfile(figuresdir1,frameName),'compact');
%     close(gcf)
end;
...
%     ylim([0 4]);    
    hold off;
    set(gcf, 'color','white', 'Position',[1 -80 1600 900], 'PaperPosition', [.25 .25 10 8], 'inverthardcopy','off')
    frameName = [strcat('D_ref_short_scale_',num2str(B.(BNames{ii,1})),'mV')];
    print(gcf, '-dpng', strcat(figuresdir1,'\',frameName),'-r0');
    savefig(gcf, fullfile(figuresdir1,frameName),'compact');
    close(gcf)
end;
...
fitidx(1,1) = find(fit_round(:,1) == 4976); %input the first boundary condition
fitidx(1,2) = find(fit_round(:,4) == 1.625e6); %input the final boundary condition
...
for ii = 1:v_max; %index across voltages
    ...
    offset_ref(ii,2) = 15; %specify curve at which to stop analysis
...
nu_0 = zeros(2,v_max); %create an array to store the attempt escape frequency 
% nu_00 = cell(t_max,v_max); %create an array to store the temperature-dependent attempt to escape frequency
...
for k = 1:t_max; %index over temperatures
    for ii = 1:v_max; %index over voltages
        ...
        AS_inv{1,ii}(k+offset_ref(ii,1),2) = reallog(ASpeaks(k+offset_ref(ii,1),ii)); %calculate ln((\omega)/T^2)
    end;
...
X_vals = linspace(1/150, 1/400, 100); %create an array of x-values for the prediction line
...
%     errbeta{1,ii} = (Xtilde.' *diag(Wprime)^(-1)* Xtilde)^(-1); %calculate the variance in the regression coefficients
    ...
    nu_0(1,ii) = (exp(abs(beta(1,ii)))/2)*2*pi; %calculate the attempt to escape frequency in Hz
    nu_0(2,ii) = (nu_0(1,ii)*fiterr(1,ii)/2)*2*pi; %calculate the 1-sigma error in the attempt to escape frequency in Hz
...
%     set(t,'Interpreter', 'Latex')
    ...
    set(x,'FontName','Calibri');
    set(gca,'FontSize',40);
    y = ylabel ('ln({\omega_0})', 'FontSize',40);
    set(y,'FontName', 'Calibri')
