...
%     frameName = [strcat('C_',num2str(B.(BNames{ii,1})),'mV')];
...
%     ylim([0 .01]);    
    hold off;
%     set(gcf, 'color','white', 'Position',[1 -80 1600 900], 'PaperPosition', [.25 .25 10 8], 'inverthardcopy','off')
%     frameName = [strcat('G-omega_ref_short_scale_',num2str(B.(BNames{ii,1})),'mV')];
%     print(gcf, '-dpng', strcat(figuresdir1,'\',frameName),'-r0');
%     savefig(gcf, fullfile(figuresdir1,frameName),'compact');
%     close(gcf)
end
...
%         ylim([0 2.7e-9]); 
        hold off;
%         set(gcf, 'color','white', 'Position',[1 -80 1600 900], 'PaperPosition', [.25 .25 10 8], 'inverthardcopy','off')
%         frameName = [strcat('smoothed_derivative_range_limit_',num2str(B.(BNames{ii,1})),'mV')];
%         print(gcf, '-dpng', strcat(figuresdir1,'\',frameName),'-r0');
%         savefig(gcf, fullfile(figuresdir1,frameName),'compact');
%         close(gcf)
end
...
for ii = 1:v_max %index across voltages
    offset_ref(ii,1) = 3; %specify the offset in curves at which to start analysis 
    offset_ref(ii,2) = 12; %specify curve at which to stop analysis
...
X_vals = linspace(1/180, 1/310, 100); %create an array of x-values for the prediction line
